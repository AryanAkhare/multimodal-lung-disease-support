...
import { ChevronLeft, ActivitySquare, AlertCircle } from 'lucide-react';
...
function App() {
  ...
  useEffect(() => {
    const checkHealth = async () => {
      ...
      if (!isHealthy && currentPage === 'app') {
        setAlertMessage('⚠️ Backend server is not responding. Please start the backend with: python backend/main.py');
      }
    };
  ...
  const handleSubmit = async () => {
    if (!imageFile && !audioFile) {
      setAlertMessage('Please provide at least an image or audio upload to run the multimodal analysis.');
      return;
    }
    ...
    // Smooth auto-scroll to top of results on mobile
        ...
        <section className="panel-premium intake-panel">
              ...
              <div className="input-group-premium full-width">
                ...
                <div className="symptoms-toggle-grid">
                  <label className="toggle-label">
                    <span>Wheeze</span>
                    <select value={tabular.wheeze} onChange={(e) => setTabular({ ...tabular, wheeze: e.target.value })}>
                      <option>No</option><option>Yes</option>
                    </select>
                  </label>
                  <label className="toggle-label">
                    <span>Crackles</span>
                    <select value={tabular.crackles} onChange={(e) => setTabular({ ...tabular, crackles: e.target.value })}>
                      <option>No</option><option>Yes</option>
                    </select>
                  </label>
                  <label className="toggle-label">
                    <span>Chest Tightness</span>
                    <select value={tabular.chest_tightness} onChange={(e) => setTabular({ ...tabular, chest_tightness: e.target.value })}>
                      <option>No</option><option>Yes</option>
                    </select>
                  </label>
                  <label className="toggle-label">
                    <span>Fever</span>
                    <select value={tabular.fever} onChange={(e) => setTabular({ ...tabular, fever: e.target.value })}>
                      <option>No</option><option>Yes</option>
                    </select>
                  </label>
                </div>
          ...
          <div className="action-bar-premium">
            ...
            <div className="button-group">
              <button className="btn-outline-premium" onClick={handleClear} disabled={loading}>
                Clear Form
              </button>
